...
run file options="":
  ...
  MODE=run-wat WAT_FILE=./build/quartz-current.wat cargo run --release -- run {{options}} {{file}}

run_2 file options="":
  @just build_current_compiler
  @just build_compiler current current.2 {{options}}
  MODE=run-wat WAT_FILE=./build/quartz-current.2.wat cargo run --release -- run {{options}} {{file}}
